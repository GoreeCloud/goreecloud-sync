...
- a Development-stage authenticated one-to-one file/text payload stream using versioned offer, receiver-decision, sender-completion, and verified-receipt records, cryptographically random transfer identifiers, bounded binary chunk frames, explicit receiver authorization before bytes are sent, and transfer-ID/size/hash-bound completion acceptance;
- application-level payload-transfer composition that requires a durable trusted-device fingerprint and can revalidate current account/device/key trust at transfer checkpoints while preserving the non-instantaneous revocation boundary;
...
Payload-transfer composition also revalidates current trust before transfer start, before sender source reads, before receiver staging writes, and before returning verified success. These remain explicit checkpoints rather than continuous revocation monitoring.
...
## Authenticated payload transfer foundation

`internal/transfer` provides a Development-stage versioned manifest for one file or logical text payload. Manifest construction streams the source payload, records its exact byte size and configured chunk size, and stores canonical lowercase SHA-256 digests for every ordered chunk and the complete payload.

The authenticated payload path uses a fresh 128-bit transfer identifier and a bounded control sequence: offer, explicit receiver decision, binary chunks, sender completion, then a verified receiver receipt. No payload bytes are sent before the receiver's application authorization callback accepts the offer. The peer must be TLS-authenticated and carry the durable trusted-device fingerprint bound by the application trust layer.

Each source chunk is checked against the manifest before transmission. Each received chunk is checked before it is written to the caller's staging destination. Sender completion and receiver receipt are bound to the exact transfer ID, declared size, and whole-payload hash. Protocol, stream, or integrity failures close the peer fail-closed rather than silently continuing on a potentially desynchronized connection.

A receive destination is staging, not an automatically authorized final filesystem path. Callers must publish or commit staged content only after verified success. This source primitive does not yet provide durable resume persistence, path authorization/confinement, LAN discovery, production address/listener/dial orchestration, transfer history, rate controls, or the user-facing Nearby workflow.

## Still incomplete

Major production and product work remains, including enabling the secure-peer factory and payload-transfer primitive in an approved runtime with discovery/address and listener/dial policy, production revalidation scheduling and revocation-aware reconnect/session behavior, broader transport/transfer replay and freshness policy beyond TLS, random transfer IDs, and the implemented one-time pairing challenge, production account/runtime wiring, user-facing pairing approval and revocation UX, LAN discovery, durable resumable file/folder synchronization, folder authorization and filesystem confinement, Nearby product workflows, Share E2EE, complete multi-user authorization and administration, Glaze UI product surfaces, Android and Debian clients, migration/rollback tooling, monitoring, deployment, and full Glaze UI/Wardveil Security/Privacy Shield/Everkeep/GoreeCloud Mesh/GoreeCloud Identity acceptance evidence appropriate to the final scope.
