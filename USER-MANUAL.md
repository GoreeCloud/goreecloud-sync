...
The default `serve` command does not automatically enable secure peer listeners, peer discovery, production account authority, Nearby, Share, or full folder synchronization.
...
Pairing proof, network reachability, a TLS session, or an authenticated application session alone does not grant every dataset or folder permission.
...
Synchronization is not backup. Everkeep remains the GoreeCloud continuity, preservation, backup, and recovery authority. A synchronized replica should not be treated as an independent recovery point unless Everkeep/backup acceptance explicitly establishes that property.
...
- LAN Nearby discovery/transfers;
