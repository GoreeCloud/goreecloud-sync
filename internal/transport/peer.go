...
func DialPeer(ctx context.Context, address string, timeout time.Duration) (*PeerConn, error) {
	if address == "" || timeout <= 0 { return nil, fmt.Errorf("peer address and timeout are required") }
	d := net.Dialer{Timeout: timeout}
	conn, err := d.DialContext(ctx, "tcp", address)
	if err != nil { return nil, fmt.Errorf("dial peer: %w", err) }
...
func (p *PeerConn) SendHandshake(handshake Handshake) error {
	...
	if err := handshake.Validate(); err != nil { return fmt.Errorf("validate local handshake: %w", err) }
	if err := WriteJSONFrame(p.conn, handshake); err != nil { return fmt.Errorf("send peer handshake: %w", err) }
	return nil
}
...
func (p *PeerConn) ReceiveHandshake() (Handshake, error) {
	if p == nil || p.conn == nil { return Handshake{}, fmt.Errorf("peer connection is unavailable") }
	var handshake Handshake
	if err := ReadJSONFrame(p.conn, &handshake); err != nil { return Handshake{}, fmt.Errorf("receive peer handshake: %w", err) }
	if err := handshake.Validate(); err != nil { return Handshake{}, fmt.Errorf("validate peer handshake: %w", err) }
