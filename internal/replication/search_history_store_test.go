...
func authorizedInputs() (datasets.RecordEnvelope, session.AuthenticatedPeer, policy.PrivacyDecision, policy.TrustEvidence) {
	...
	privacy := policy.PrivacyDecision{
		Purpose: "cross-device-search-history", PurposeAllowed: true, ConsentGranted: true,
		DecidedAt: when, EvidenceID: "privacy-evidence",
	}
	trust := policy.TrustEvidence{
		DeviceID: "device-a", KeyFingerprint: "fingerprint-a", Trusted: true,
		EvaluatedAt: when, EvidenceID: "trust-evidence",
	}
...
func TestAcceptAndPersistWritesOnlyAuthorizedHistory(t *testing.T) {
	...
	if len(records) != 1 || records[0].RecordID != "history-1" {
...
func TestRejectedRecordNeverPersists(t *testing.T) {
	...
	if len(records) != 0 {
		t.Fatalf("rejected record persisted: %+v", records)
	}
}
...
func TestHigherRevisionWinsPersistenceConflict(t *testing.T) {
	...
	if _, err := store.AcceptAndPersist(record, peer, privacy, trust, time.Unix(202, 0).UTC()); err != nil {
		t.Fatal(err)
	}
	records, err := store.Records()
	if err != nil {
	...
	if len(records) != 1 || records[0].Revision != 2 {
		t.Fatalf("unexpected conflict result: %+v", records)
	}
}
