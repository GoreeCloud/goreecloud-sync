...
var ErrBookmarkItemsOnly = errors.New("replication store accepts only bookmarks.items")

type BookmarkItemStore struct {
	mu   sync.Mutex
	path string
}

func NewBookmarkItemStore(path string) *BookmarkItemStore { return &BookmarkItemStore{path: path} }

func (s *BookmarkItemStore) AcceptAndPersist(record datasets.RecordEnvelope, peer session.AuthenticatedPeer, privacy policy.PrivacyDecision, trust policy.TrustEvidence, now time.Time) (policy.AcceptanceEvidence, error) {
	if record.Dataset != "bookmarks.items" {
		return policy.AcceptanceEvidence{}, ErrBookmarkItemsOnly
	}
...
func (s *BookmarkItemStore) Records() ([]datasets.RecordEnvelope, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	records, err := s.load()
	if err != nil {
...
func (s *BookmarkItemStore) save(records map[string]persistedRecord) error {
