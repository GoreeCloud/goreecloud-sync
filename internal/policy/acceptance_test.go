...
func testInputs() (datasets.RecordEnvelope, session.AuthenticatedPeer, PrivacyDecision, TrustEvidence) {
	...
	record := datasets.RecordEnvelope{
		...
		UpdatedAt:     when,
		OriginDevice:  "peer-a",
		Payload:       map[string]any{"query": "weather"},
	}
	peer := session.AuthenticatedPeer{
		DeviceID:       "peer-a",
		KeyFingerprint: "fp-a",
		NegotiatedDatasets: []datasets.Capability{{
			Dataset: "search.history", SchemaVersion: 1, Read: true, Write: true, Delete: true,
		}},
	}
	privacy := PrivacyDecision{
		Purpose: "cross-device-search-history", PurposeAllowed: true, ConsentGranted: true,
		DecidedAt: when, EvidenceID: "privacy-1",
	}
	trust := TrustEvidence{
		DeviceID: "peer-a", KeyFingerprint: "fp-a", Trusted: true,
		EvaluatedAt: when, EvidenceID: "trust-1",
	}
...
func TestAuthorizeRecordRequiresPrivacyAndTrust(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("AuthorizeRecord: %v", err)
	}
	if evidence.PrivacyEvidenceID != "privacy-1" || evidence.TrustEvidenceID != "trust-1" {
...
func TestAuthorizeRecordFailsClosed(t *testing.T) {
	...
	if _, err := AuthorizeRecord(record, peer, privacy, trust, time.Time{}); !errors.Is(err, ErrConsentDenied) {
		t.Fatalf("error = %v", err)
	}
	privacy.ConsentGranted = true
	trust.Trusted = false
	if _, err := AuthorizeRecord(record, peer, privacy, trust, time.Time{}); !errors.Is(err, ErrPeerUntrusted) {
		t.Fatalf("error = %v", err)
	}
}
...
func TestAuthorizeRecordRequiresNegotiatedDataset(t *testing.T) {
	...
	if _, err := AuthorizeRecord(record, peer, privacy, trust, time.Time{}); !errors.Is(err, ErrDatasetUnauthorized) {
		t.Fatalf("error = %v", err)
	}
}
